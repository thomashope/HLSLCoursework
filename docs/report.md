...
A second pass is done where the normals of the Magic Sphere are calculated in the domanin shader and rendered as colours to a buffer. Creating something akin to a normal.

When blending the Magic Sphere into the scene the alpha channel is sampled to detect if the sphere is visible or obstructed, if the magic sphere is visible then the pixel is blended with the scene behind, otherwise the scene's pixel is returned unmodified. When sampling the scene behind for blending the magic sphere's normal map is used to offset the sample, giving the appearence of refraction.
